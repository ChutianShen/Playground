...
import com.example.kevin_sct.beastchat.Config;
import com.example.kevin_sct.beastchat.R;
import com.example.kevin_sct.beastchat.adapter.DeviceshowAdapter;
import com.example.kevin_sct.beastchat.bean.Device;
...
public class BlueToothFindOthersAty extends Activity {
    ...
    private void initView() {
        ...
        final String benjiname = mbluetoothAdapter.getName();//本机名称
        String benjidizhi = mbluetoothAdapter.getAddress();//本机地址
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    //等待客户端接入
                    while (true) {
                        bluetoothServerSocket = mbluetoothAdapter.listenUsingInsecureRfcommWithServiceRecord(benjiname, Config.UUID);
    ...
    private void buildConnect(int position, boolean isfaqiren) {
        //自己主动去连接
        BluetoothDevice device = mbluetoothAdapter.getRemoteDevice(deviceNameAndDresss.get(position).getDeviceAddress());
